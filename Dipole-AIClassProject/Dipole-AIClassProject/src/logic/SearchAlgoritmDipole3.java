...
public class SearchAlgoritmDipole3{
	...
	public float ChessboardEvaluation(Chessboard board,boolean maximizing) {// anche se è sbagliato si sta ssumendo per ora
        // TODO Auto-generated method stub                          //che il colore 0=white=maximizer e l'opposto 
        float score=0;                                              //per colore 1,probabilmente conviene invertire
        ...
        while(i<Chessboard.SIDE_LENGTH*Chessboard.SIDE_LENGTH) {
            ...
            if(chessboard[i] !=20 && chessboard[i] !=0) {
                ...
                if(chessboard[sourceIndex]<0) {
                    ...
                    score+=scoreFromEating(board,sourceIndex,allLecitMoves,true);
    ...
    private float scoreFromEating(Chessboard board, int sourceIndex, ArrayList<Move> allLecitMoves,boolean calcBest) {
        float score=0;
        int numberPawns;
        int[] chessboard=board.getChessboard(); 
        for(int j=0; j<allLecitMoves.size();j++) {
            ...
            if(m.getType()==MoveType.BACKWARD_ATTACK) {
                score+=chessboard[dIndex]*-24;
                if(calcBest) {//calcola la probabile mossa che farà solo in base a chi chiama il minmax,se lo chiama il bianco allora valuto una scacchiera per il bianco
                    numberPawns=Math.abs(chessboard[dIndex]);
                    if(numberPawns>bestEat)bestEat=numberPawns;
                    }
                //score+=scoreFromCounterEating(board,dIndex, distance,sourceIndex);
             }
            if(m.getType()==MoveType.FORWARD_ATTACK) {
                score+=chessboard[dIndex]*-20;
                if(calcBest) {
                    numberPawns=Math.abs(chessboard[dIndex]);
                    if(numberPawns>bestEat)bestEat=numberPawns;
                    }
